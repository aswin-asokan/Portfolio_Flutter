    ...
    if (!_hasShownToast) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          showToast(
            context,
            "Work in Progress, Prone to changes. Continuously shipping questionable decisions!",
          );
        }
      });
    }
          ...
          SingleChildScrollView(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(height: 90, key: sectionKeys[0]), // Space for navbar
                  RepaintBoundary(
                    child: HeroSection(
                  ...
                  const RepaintBoundary(child: TechIUse()),
                  if (MediaQuery.sizeOf(context).width >= 1024) ...[
                    Builder(
                      builder: (context) {
                        final double contentWidth =
                            MediaQuery.sizeOf(context).width - 2 * padding;
                        final double normalProjectsWidth =
                            (contentWidth - 20) / 2 - 48;
                        final double expandedProjectsWidth = contentWidth - 48;

                        final double expLeft = 0;
                        final double expTop = 0;
                        final double expWidth =
                        ...
                        final double projLeft =
                            _isExperienceExpanded
                                ? 0
                                : (contentWidth - 20) / 2 + 20;
                        final double projTop =
                            _isExperienceExpanded ? expHeight + 20 : 0;
                        final double projWidth =
                            _isExperienceExpanded
                                ? contentWidth
                                : (contentWidth - 20) / 2;
                        final double projHeight =
                            _isExperienceExpanded ? 520.0 : 520.0;
                    ...
                    const RepaintBoundary(child: Education()),
                  ] else ...[
                    AnimatedSize(
                      duration: const Duration(milliseconds: 500),
                      curve: Curves.easeInOut,
                    ...
                    RepaintBoundary(child: Projects(key: sectionKeys[3])),
                    const RepaintBoundary(child: Education()),
                  ],
                  RepaintBoundary(
                    child: Column(
                  ...
                  RepaintBoundary(child: ContactMe(key: sectionKeys[4])),
